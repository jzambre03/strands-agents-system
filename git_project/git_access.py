...
def test_gitlab_access(token: str, repo_url: str):
    """
    ...
    project_path = parse_repo_url(repo_url)
    if not project_path:
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if 'has already been taken' in str(error_msg):
                ...
                search_url = f"https://gitlab.com/api/v4/projects?search={project_name}"
...
def copy_config_files_to_new_repo(
    ...
    try:
        ...
        source_project_path = parse_repo_url(source_repo_url)
        ...
        new_project = create_gitlab_project(token, new_project_name, namespace, base_url=base_url)
        if not new_project:
...
def main():
    ...
    # Test access
    success = test_gitlab_access(config['gitlab_token'], config['gitlab_url'])
